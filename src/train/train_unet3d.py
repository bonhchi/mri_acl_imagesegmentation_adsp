...
class KneeNPZ3D(Dataset):
    ...
    def __init__(
        ...
        self.files = [
            l.strip() for l in open(list_txt, "r", encoding="utf-8") if l.strip()
        ]
        self.roi = tuple(roi)
    ...
    def _load(self, p):
        z = np.load(p)
        x = z["img"].astype(np.float32)
        y = z["msk"].astype(np.int64)
...
def main():
    ...
    train_ds = KneeNPZ3D(
        args.train_list, roi=tuple(args.roi), samples_per_vol=12, train=True
    )
    val_ds = KneeNPZ3D(
        args.val_list, roi=tuple(args.roi), samples_per_vol=2, train=False
    )
    train_ld = DataLoader(
        train_ds,
    ...
    model = build_unet3d(
        in_ch=1, classes=args.classes, channels=tuple(args.channels)
    ).to(device)
    loss = (
    ...
    best = 1e9
    best_path = os.path.join(args.out_dir, "best3d.pt")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(
            f"Epoch {ep:03d}/{args.epochs} | train {tr:.4f} | val {vl:.4f} | lr {lr:.2e}"
        )
